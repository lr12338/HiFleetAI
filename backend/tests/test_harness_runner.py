...
def test_handoff_category_runs_real_api_sequence(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("HARNESS_REPORT_DIR", str(tmp_path))

    exit_code = run_eval.main(["--category", "handoff"])
    ...
    assert report["summary"] == {"total": 1, "passed": 1, "failed": 0, "pass_rate": 1.0}
    result = report["results"][0]
    assert result["case_id"] == "handoff_001"
    assert result["passed"] is True
    assert result["agent_result"]["response"]["handoff_status"] == "ai_active"
    assert result["agent_result"]["response"]["sequence"] == [
        {
            "action": "handoff",
            "http_status": 200,
            "expected_handoff_status": "human_active",
            "handoff_status": "human_active",
            "payload": {
                "conversation_id": result["agent_result"]["response"]["conversation_id"],
                "handoff_status": "human_active",
                "assigned_agent_id": result["agent_result"]["response"]["sequence"][0]["payload"]["assigned_agent_id"],
                "event_type": "takeover",
            },
        },
        {
            "action": "pause-ai",
            "http_status": 200,
            "expected_handoff_status": "ai_paused",
